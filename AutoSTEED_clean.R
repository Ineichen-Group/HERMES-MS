...
MSmodel_regex <- c("cuprizone", "\\bEAE\\b", "experimental autoimmune encephalomyelitis",
                   ...
                   "JME", "lipopolysaccharide", "TMEV", "cytokine injection", "hyponatr(a*)emia", "NMO",
                   "optic neuritis", "ethidium bromide", "EtBr")

species_list <- c("\\brat[^ihe]", "\\bmarmoset(s*)", "\\bguinea(| )pig(s*)", "\\bmacaque(s*)", "\\brhesus(| )monkey(s*)",
                  "\\bswine(s*)", "\\bdog(s*)", "\\bcat(s|\\b)[^.]", "\\brabbit(s*)", "\\bprimate(s*)",
                  "\\bhamster(s*)", "\\bsheep", "\\bmouse", "\\bmice", "mini( *)pig")
...
regex_removal_rob <- "powerful|power level|powerlevel|wave power|wavepower|random effect(s*)|
                      random primer(s*)|random hexamer(s*)|random variable(s*)|random coefficient(s*)|
                      random variation|random network|randomized-controlled|randomized controlled|
                      randomized control|blindness|double(-*| )blind"
...
pdf.rob.vector <- list.files("PDFs_included_studies/", full.names = T)
...
mined.rob <- vector()
mined.rob.end <- vector()

for(pdf.i in 1:lenght(pdf.rob.vector)) {
  print(pdf.i)
  print(pdf.rob.vector[pdf.i])
  mined.rob_end <-  rbind(mined.rob.end, pdf.extraction(src =pdf.rob.vector[pdf.i]))
}

#Extracted data (as dataframe)
mined.rob_df <- mined.rob %>% data.frame()

df_last_files<-rbind(pdf.extraction(src = "PDFs_included_studies/Perrin 1995.pdf"),
      pdf.extraction(src = pdf.rob.vector[428]),
      pdf.extraction(src = pdf.rob.vector[429]),
      pdf.extraction(src = pdf.rob.vector[430]),
      pdf.extraction(src = pdf.rob.vector[431]),
      pdf.extraction(src = pdf.rob.vector[432]),
      pdf.extraction(src = pdf.rob.vector[433]),
      pdf.extraction(src = pdf.rob.vector[434]),
      pdf.extraction(src = pdf.rob.vector[435])) %>% data.frame()

names_end_files<-rbind(pdf.rob.vector[427],
       pdf.rob.vector[428],
       pdf.rob.vector[429],
       pdf.rob.vector[430],
       pdf.rob.vector[431],
...
df_last_files$File.title<-names_end_files

mined_rob<-rbind(mined.rob_df,df_last_files)

# write_xlsx(mined_rob,"mined_rob.xlsx")
