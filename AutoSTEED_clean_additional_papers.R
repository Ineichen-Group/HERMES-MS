...
pdf.rob.vector <- list.files("Additional_papers/", full.names = T)
...
mined.rob_end<-data.frame()
for(i in pdf.rob.vector){
  print(i)
  mined.rob<-
    data.frame(pdf.extraction(i)) %>%
    rename(value=1) %>% 
    rownames_to_column() %>% 
    pivot_wider(names_from=rowname,values_from=value) %>% 
    mutate(`File title`=i)
  
  mined.rob_end<-rbind(mined.rob_end,mined.rob)
}

mined.rob_additional_papers<-mined.rob_end%>%
  rename_with(~ gsub(" ", ".", .x))

rbind(mined_rob,mined.rob_additional_papers)

write_xlsx(rbind(mined_rob,mined.rob_additional_papers),"mined_rob_with_addpaper.xlsx")
