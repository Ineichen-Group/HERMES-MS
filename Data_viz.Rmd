...
drug_info_df<-merge(rbind(HERMES_INCLUDED %>% group_by(Tested.drug.2,status2) %>% count() %>% rename(drugs=1,status=2),
...
ggplot() +
  ...
  facet_wrap(.~drug_status, scales="free_y",ncol=1)
...
Figure 4 - FDA/first-in-human panel - DONE
