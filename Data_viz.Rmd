...
ggsave("Figure2_final_longnames_updated.png",figure_panel3,height = 29/2.5,width = 21/2.5)
ggsave("Figure2_final_longnames_updated.pdf",figure_panel3,height = 29/2.5,width = 21/2.5)
ggsave("Figure2_final_longnames_updated.tiff",dpi=300,figure_panel3,height = 29/2.5,width = 21/2.5)
```
...
ggsave("Figure6_timeline_final_strip_right_updated.png",panel_fig_FDA,height = 29/2.7,width = 21/2.7,bg = "white")
ggsave("Figure6_timeline_final_strip_right_updated.pdf",panel_fig_FDA,height = 29/2.7,width = 21/2.7,bg = "white")
ggsave("Figure6_timeline_final_strip_right_updated.tiff",dpi=300,panel_fig_FDA,height = 29/2.7,width = 21/2.7,bg = "white")
...
ggsave("Figure4_ES_final_updated.png",MRI_EAE_ES_plot,height = 29/3,width = 21/3,bg = "white")
ggsave("Figure4_ES_final_updated.pdf",MRI_EAE_ES_plot,height = 29/3,width = 21/3,bg = "white")
ggsave("Figure4_ES_final_updated.tiff",dpi=300,MRI_EAE_ES_plot,height = 29/3,width = 21/3,bg = "white")

ES_df_trial2 %>% filter(color=="overall") %>% ungroup() %>% 
  select(item,drug_status,yi,lower_CI,upper_CI)
```

#Rationale of studies - pia's extraction
```{r message=FALSE, warning=FALSE,fig.width=5,fig.height=7}
justification_beforeapproval_df <-
  read_csv("justification_beforeapproval_df.csv") %>% 
  mutate(df="justification_beforeapproval_df")%>% 
  rename(item=1) %>% arrange(Freq) %>% mutate(order_y=1:n()+20)
justification_allstudies_df <-
  read_csv("justification_allstudies_df.csv")%>% 
  mutate(df="justification_allstudies_df")%>% 
  rename(item=1) %>% arrange(Freq) %>% mutate(order_y=1:n())
justification_afterapproval_df <-
  read_csv("justification_afterapproval_df.csv")%>% 
  mutate(df="justification_afterapproval_df") %>% 
  rename(item=1) %>% arrange(Freq) %>% mutate(order_y=1:n()+40)

plot_df_motivation<-rbind(justification_beforeapproval_df,justification_allstudies_df,justification_afterapproval_df) %>% 
  mutate(df=case_when(
    df=="justification_beforeapproval_df"~"Before approval",
    df=="justification_allstudies_df"~"All studies",
    df=="justification_afterapproval_df"~"After approval",
  )) %>% 
  arrange(order_y) %>% mutate(order_y=1:n())

all_studies_plot<-
  plot_df_motivation %>% 
  filter(df=="All studies")  %>%  
ggplot(aes(x=Freq,y=reorder(item,order_y)))+
  geom_bar(stat = "identity", fill = "#b6b6b6",color="#929292")+
  common_theme+
  theme(
  strip.text.x = element_text(hjust=0,size=12),
  ...
  facet_wrap(df~.,scales="free",ncol=1, strip.position="right")
   
after_approval_plot<-
  plot_df_motivation %>% 
  filter(df=="After approval")  %>%  
ggplot(aes(x=Freq,y=reorder(item,order_y)))+
  geom_bar(stat = "identity", fill = "#b6b6b6",color="#929292")+
  common_theme+
  theme(
...
before_approval_plot<-
  plot_df_motivation %>% 
  filter(df=="Before approval")  %>%  
ggplot(aes(x=Freq,y=reorder(item,order_y)))+
  geom_bar(stat = "identity", fill = "#b6b6b6",color="#929292")+
  common_theme+
  theme(
...
motivationplots_arranged<-ggarrange(all_studies_plot,before_approval_plot,after_approval_plot,ncol=1,heights = c(1,0.8,1))

ggsave("motivation_plot.png",motivationplots_arranged,height = 28/4,width = 20/4,bg = "white")
ggsave("motivation_plot.pdf",motivationplots_arranged,height = 28/4,width = 20/4,bg = "white")
ggsave("motivation_plot.tiff",dpi=300,motivationplots_arranged,height = 28/4,width = 20/4,bg = "white")
