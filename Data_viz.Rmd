...
justification_beforeapproval_df <- read_csv("justification_beforeapproval_df.csv") %>% 
  mutate(df="justification_beforeapproval_df")%>% 
  rename(item=1)
justification_allstudies_df <- read_csv("justification_allstudies_df.csv")%>% 
  mutate(df="justification_allstudies_df")%>% 
  rename(item=1)
justification_afterapproval_df <- read_csv("justification_afterapproval_df.csv")%>% 
  mutate(df="justification_afterapproval_df") %>% 
  rename(item=1)

rbind(justification_beforeapproval_df,justification_allstudies_df,justification_afterapproval_df) %>% 
  mutate(df=case_when(
  ...
  arrange(df,Freq) %>% 
  mutate(order_y=paste0("ord",1:n())) %>% 
  ggplot(aes(x=Freq,y=reorder(item,Freq)))+
  geom_bar(stat = "identity", fill = "#b6b6b6",color="#929292")+
  common_theme+
  theme(
  strip.text.x = element_text(hjust=0,size=12),
