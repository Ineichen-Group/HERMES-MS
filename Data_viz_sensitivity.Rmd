...
Data prep
```{r message=FALSE, warning=FALSE, fig.show='asis'}
fig_panelDescr<-list()
for(i in c("Randomization","Blinding","Both")){

HERMES_INCLUDED <- read.csv("HERMES_INCLUDED_sensitivity.csv")%>%
  mutate(subset_sensitivity=case_when(
    subset_sensitivity=="rand"~"Randomization",
    subset_sensitivity=="blind"~"Blinding",
    subset_sensitivity=="both"~"Both"
  )) %>% 
      filter(subset_sensitivity==i) %>% #rand blind both
...
df_row2<-rbind(HERMES_INCLUDED %>%
                 filter(exclude_duplicate_study=="no") %>% 
  mutate(Sex=str_to_title(gsub("\\bboth\\b","both sexes",Sex.x))) %>% 
  group_by(Sex) %>%
  summarise(Frequency = n()) %>%
  ungroup() %>% 
...
combined_plot <- grid.arrange(plot_row1,plot_row2,
                            p3+ggtitle(bquote(bold("e.")~"Tested drugs"))+
                              theme(legend.position = "bottom",
                                    legend.text = element_text(size=13,color="gray20")),
                            ncol=1,heights=c(1.1,1,1.5))

overall_title <- textGrob(i, gp = gpar(fontsize = 16, fontface = "bold"), vjust = 1.5)


fig_panelDescr[[i]]<-grid.arrange(arrangeGrob(overall_title = overall_title, combined_plot, nrow = 2, heights = c(0.1, 1)))
}



ggsave("SensitivityAnalysis/Figure2_final_longnames_rand.tiff",ggarrange(fig_panelDescr[["Both"]],
          fig_panelDescr[["Blinding"]],
          fig_panelDescr[["Randomization"]],nrow=1),height = 29/2.5,width = 40/2.5,bg="white")
```
...
fig_panelFDA[[i]]<-grid.arrange(arrangeGrob(overall_title = overall_title, panel_fig_FDA, nrow = 2, heights = c(0.1, 1)))
}


ggsave("SensitivityAnalysis/Figure6_timeline_sensitivity.tiff",ggarrange(fig_panelFDA[["Both"]],
          fig_panelFDA[["Blinding"]],
          fig_panelFDA[["Randomization"]],nrow=1),height = 29/2.5,width = 40/2.5,bg="white")
...
fig_panelES[[i]]<-grid.arrange(arrangeGrob(overall_title = overall_title, MRI_EAE_ES_plot, nrow = 2, heights = c(0.1, 1)))
}


ggsave("SensitivityAnalysis/FigureES_sensitivity.tiff",ggarrange(fig_panelES[["Both"]],
          fig_panelES[["Blinding"]],
          fig_panelES[["Randomization"]],nrow=1),height = 29/2.5,width = 40/2.5,bg="white")
