...
```{r}
MS_translation_extraction <- read_excel("MS_translation_extraction.xlsx", 
...
merge(RoB_manual_clean %>% select(DOI,paste0("Conflict","_manual")) %>% rename(manual=2),
RoB_mined_clean %>% select(DOI,paste0("Conflict","_mined"))%>% rename(mined=2),by="DOI") %>% 
  mutate(result=case_when(
    manual==1&mined==1~"TP",
    manual==1&mined==0~"FN",
    manual==0&mined==1~"FP",
...
RoB_mined_clean %>% 
  ...
  scale_fill_manual("Risk of Bias",
          values = c(High = "#bf0000", Low = "#02c100"),
